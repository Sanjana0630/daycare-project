            ...
            {type === 'select' ? (
                <div className="relative">
                    <select
                        ...
                        onChange={onChange}
                        onBlur={onBlur}
                        className={`w-full ${Icon ? 'pl-10' : 'pl-4'} pr-10 py-2.5 bg-white border ${error ? 'border-red-500 ring-1 ring-red-100' : 'border-gray-200'} rounded-xl focus:ring-2 ${error ? 'focus:ring-red-100 focus:border-red-500' : 'focus:ring-purple-100 focus:border-purple-400'} transition-all outline-none appearance-none cursor-pointer`}
                    >
...
const EditChild = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        Object.keys(formData).forEach(key => {
            // Skip validating photo if not changed
